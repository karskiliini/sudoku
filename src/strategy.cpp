...
bool foundOnlyOnceNinth(Board *board, Ninth *ninth, uint32_t value) {
  ...
  for (uint32_t y = ninth->getCoord().y * 3; y < ninth->getCoord().y * 3 + 3;
       ++y) {
    for (uint32_t x = ninth->getCoord().x * 3; x < ninth->getCoord().x * 3 + 3;
         ++x) {
        ...
        for (uint32_t nx = 0; nx < 3; ++nx) {
          ...
          for (uint32_t x = 0; x < 9; ++x) {
            if (foundOnlyOnCol(board, ninth, x, check)) {
              for (uint32_t y = 0; y < 9; ++y) {
                auto ninthToClear = board->getNinth(x, y);
                if (ninthToClear->getCoord() == ninth->getCoord())
                  continue;
  ...
  for (uint32_t index1 = 0; index1 < line.size() - 1; ++index1) {
    ...
    if (line[index1]->hasValue(value) && (line[index1]->count() > 1)) {
      ...
      for (uint32_t index2 = index1 + 1; index2 < line.size(); ++index2) {
        if (line[index2]->hasValue(value) && (line[index2]->count() > 1)) {
          found2 = index2;

          for (uint32_t index3 = index2 + 1; index3 < line.size(); ++index3) {
  ...
  do {
...

    {
      for (uint32_t check = 1; check <= 9; ++check) {
        ...
        for (uint32_t x = 0; x < (9 - (COUNT - 1)) && (found3.found == false);
             ++x) {
          ...
          if (found1.found) {
            for (uint32_t x2 = x + 1;
                 x2 < (9 - (COUNT - 2)) && (found3.found == false); ++x2) {
              auto col2 = board->getCol(x2);
              found2 = findDoubles(col2, check, x2);
              if (found2.found && (found1.index1 == found2.index1) &&
...
#endif
                if (found1.found && found2.found && !found3.found) {
                  for (uint32_t x = 0; x < 9; ++x) {
                    if (x != found1.row_or_col && x != found2.row_or_col) {
                      assert(found1.row_or_col != found2.row_or_col);
                      assert(found1.index1 == found2.index1);
                      assert(found1.index2 == found2.index2);
                      run |= board->at(x, found1.index1)->Remove(found1.value);
                      run |= board->at(x, found1.index2)->Remove(found1.value);
                    }
                  }
                }
              }
...

    {
      for (uint32_t check = 1; check <= 9; ++check) {
        ...
        for (uint32_t y = 0; y < (9 - (COUNT - 1)) && (found3.found == false);
             ++y) {
          ...
          if (found1.found) {
            for (uint32_t y2 = y + 1;
                 y2 < (9 - (COUNT - 2)) && (found3.found == false); ++y2) {
              ...
              if (found2.found && (found1.index1 == found2.index1) &&
                ...
                if (found1.found && found2.found && !found3.found) {
                  for (uint32_t y = 0; y < 9; ++y) {
                    if (y != found1.row_or_col && y != found2.row_or_col) {
                      assert(found1.row_or_col != found2.row_or_col);
                      assert(found1.index1 == found2.index1);
                      assert(found1.index2 == found2.index2);
                      run |= board->at(found1.index1, y)->Remove(found1.value);
                      run |= board->at(found1.index2, y)->Remove(found1.value);
                    }
                  }
                }
              }
